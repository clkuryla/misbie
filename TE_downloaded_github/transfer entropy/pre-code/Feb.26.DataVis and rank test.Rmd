...
```{r load dataset}
trans_res_post_13 = read.csv(file = "/Users/yangtianrun/Desktop/Pei group/Yan_2023/Load_data/trans_res_post_13.csv") %>% 
  select(-num_obs,-X) %>% 
  rename(a_Mutation = a, 
         b_Mutation = b, 
         c_Mutation = c, 
...
trans_res_post_2 =read.csv(file = "/Users/yangtianrun/Desktop/Pei group/Yan_2023/Load_data/trans_res_post_2.csv")%>% 
  select(-num_obs, -X) %>% 
  rename(a_Deletion = a, 
         b_Deletion = b, 
         c_Deletion  = c, 
...
trans_res_post_control = read.csv(file = "/Users/yangtianrun/Desktop/Pei group/Yan_2023/Load_data/trans_res_control.csv")%>% 
  select(-num_obs,-X) %>% 
  rename(a_Control = a, 
         b_Control = b, 
         c_Control  = c, 
...
overview  %>%
  ...
  facet_grid(Status ~ Type, scales = "free_x")
