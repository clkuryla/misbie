...
rhrv_function(hr_cleaned %>% filter(id == "mi002") %>%  filter(period == 10) %>% pull(pSec),
              int_method = "spline", freqhr = 4)

hr_rhrv_summary <- hr_cleaned %>% filter(period != "Acquisition") %>% filter(period != "C3") %>% filter(period != "Preptask") %>% 
  group_by(id, period) %>% 
  summarize(hr_SDNN = rhrv_function(pSec, int_method = "spline", freqhr = 4)["SDNN"],
            hr_SDANN = rhrv_function(pSec, int_method = "spline", freqhr = 4)["SDANN"],
            hr_SDNNIDX = rhrv_function(pSec, int_method = "spline", freqhr = 4)["SDNNIDX"],
            hr_pNN50 = rhrv_function(pSec, int_method = "spline", freqhr = 4)["pNN50"],
            hr_SDSD = rhrv_function(pSec, int_method = "spline", freqhr = 4)["SDSD"],
            hr_rMSSD = rhrv_function(pSec, int_method = "spline", freqhr = 4)["rMSSD"],
            hr_IRRR = rhrv_function(pSec, int_method = "spline", freqhr = 4)["IRRR"],
            hr_MADRR = rhrv_function(pSec, int_method = "spline", freqhr = 4)["MADRR"],
            hr_TINN = rhrv_function(pSec, int_method = "spline", freqhr = 4)["TINN"],
            hr_HRVi = rhrv_function(pSec, int_method = "spline", freqhr = 4)["HRVi"]
            ) %>% 
...
wilcox.test(hr_HRVi ~ condition, hr_rhrv_summary %>% filter(condition %in% c("Control", "Deletion")))

wilcox.test(hr_pNN50 ~ condition, hr_rhrv_summary %>% filter(condition %in% c("Control", "Mutation")))
